...
import com.example.wxhk.msg.impl.*;
...
public class WxMsgHandle {
    public static final ConcurrentHashMap<Integer, IMsgHandler> map = new ConcurrentHashMap<>(32);
...
//        }, WxMsgType.扫码触发);
    ...
    public static void exec(PrivateChatMsg chatMsg) {
        IMsgHandler handle = map.get(chatMsg.getType());
        if (handle != null) {
            handle.handle(chatMsg);
        }
    }

    public void add(IMsgHandler handle, WxMsgType... type) {
